                ...
                <div class="col-8">
                    @if ($orders[0]->status == 'Order Confirmed')
                        <div class="card">
                            ...
                            <div class="card-body">
                                <p>Order has been confirmed by the buyer. You can now print the receipt and pack the orders.
                                </p>
                                <form action="{{ route('order.update.status', $order_number) }}" name="form-order-status"
                                    method="post" class="btn">
                                    @csrf
                                    <input type="text" value="Cancel" name="newOrderStatus" hidden>
                                    <button class="btn btn-danger">Cancel Order</button>
                                </form>
                                <a href="{{ route('order.print', $order_number) }}" class="btn btn-secondary"
                                    target='_blank'>Print Packing List</a>
                                <form action="{{ route('order.update.status', $order_number) }}" name="form-order-status"
                                    method="post" class="btn">
                                    @csrf
                                    <input type="text" value="To Pack" name="newOrderStatus" hidden>
                                    <button class="btn btn-success">To Pack</button>
                                </form>
                            </div>
                        </div>
                    @elseif ($orders[0]->status == 'To Pack')
                        <div class="card">
                            ...
                            <div class="card-body">
                                <p>Buyer is waiting for you to ship. Orders are currently being packed.</p>
                                <form action="{{ route('order.update.status', $order_number) }}" name="form-order-status"
                                    method="post" class="btn">
                                    @csrf
                                    <input type="text" value="Cancel" name="newOrderStatus" hidden>
                                ...
                                <a href="{{ route('order.print', $order_number) }}" class="btn btn-secondary"
                                    target='_blank'>Reprint Packing List</a>
                                @if ($orders[0]->user_addresses_id === null)
                                    <form action="{{ route('order.update.status', $order_number) }}"
                                        name="form-order-status" method="post" class="btn">
                                        @csrf
                                        <input type="text" value="Ready for Pick Up" name="newOrderStatus" hidden>
                    ...
                    @elseif ($orders[0]->status == 'Ready for Pick Up')
                        <div class="card">
                            ...
                            <div class="card-body">
                                <p>This order are ready for pick up in store location. Waiting for the customer to arrive.
                                </p>
                            </div>
                        </div>
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-6">
                                    <p>For buyers with low % of successful deliveries, you might want to contact them to
                                        confirm the order.</p>
                                </div>
                                <div class="col-6">
            ...
            <div class="row">
                <div class="col-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <p>&emsp;{{ $orders[0]->getUserRelation->display_name }} -
                                {{ $orders[0]->getUserRelation->mobile }}
                        </div>
                ...
                <div class="col-8">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <p>{{ $orders[0]->deliveryAddress ? $orders[0]->deliveryAddress->getFormattedAddress() : 'Pickup in Store Location' }}
                            </p>
                        </div>
                    </div>
                </div>
